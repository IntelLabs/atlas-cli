...
const ATLAS_CLI_BUILDER_ID: &str = "https://github.com/IntelLabs/atlas-cli";

pub fn make_build_definition_v1(build_type: &str,
    ...
    build_def.external_parameters = Some(external_parameters.clone()).into();

    // this field is optional for all SLSA Build levels
    if internal_parameters.is_some() {
	build_def.internal_parameters = Some(internal_parameters.unwrap().clone()).into();
    }
...
pub fn make_builder_v1(id: &str, builder_dependencies: Option<&[ResourceDescriptor]>,
		   version: Option<&HashMap<String, String>>) -> provenance::Builder {
...
pub fn make_build_metadata_v1(invoction_id: &str, started_on: &str, finished_on: &str)
    ...
    build_metadata.started_on = started_on.to_string();
    build_metadata.finished_on = finished_on.to_string();

    build_metadata
}

pub fn make_run_details_v1(builder: &provenance::Builder,
    ...
    run_details = Some(builder.clone()).into();

    // this field is optional for all SLSA Build levels
    if metadata.is_some() {
	run_details.metadata = Some(metadata.unwrap().clone()).into();
    }
...
pub fn generate_build_provenance_v1(build_definition: &provenance::BuildDefinition,
				    run_details: &provenace::RunDetails) -> provenance::Provenance {
    let mut provenance = provenance::Provenance::new();
    provenance.build_definition = Some(build_definition.clone()).into();
    provenance.run_details = Some(run_details.clone()).into();
